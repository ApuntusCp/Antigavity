...
export const metadata = {
  ...
  description: "Actualizaciones, avances y reflexiones sobre el Movimiento Solar Punk de GranColinos.",
};

export default async function MovimientoBlogIndex() {
  const posts = await fetchBlogPosts('movimiento');
  return (
    <div className="max-w-7xl mx-auto px-6 py-20">
      <div className="text-center mb-24 fade-in">
        ...
        <p className="text-gray-500 text-lg max-w-2xl mx-auto font-light">
          Registros, avances y la voz de la revolución Solar Punk.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
        {posts.length === 0 ? (
          <div className="col-span-full py-20 text-center">
            <p className="text-gray-500 font-sans tracking-widest uppercase text-sm">No hay publicaciones disponibles para el movimiento.</p>
          </div>
        ) : (
          posts.map((post, i) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className={`group fade-in delay-${(i % 4 + 1) * 100} block`}>
              ...
              <div className="flex flex-col">
                <span className="text-brand-green text-[10px] font-bold tracking-[0.2em] uppercase mb-4">
                  {post.date}
                </span>
                <h2 className="font-playfair text-2xl md:text-3xl text-brand-dark dark:text-white mb-4 group-hover:text-brand-green transition-colors duration-300">
                  {post.title}
                </h2>
