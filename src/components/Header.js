...
export default function Header({ headerConfig = {} }) {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-5 left-0 right-0 z-50 px-4 md:px-8 max-w-7xl mx-auto pointer-events-none">
        
        {/* Mobile Dropdown Navigation (Renders ABOVE bottom navbar when open) */}
        {isMobileMenuOpen && (
        ...
        <header className="pointer-events-auto nav-pill-bottom-floating px-6 py-3.5 flex items-center justify-between transition-all duration-300">
          ...
          <nav className="hidden md:flex items-center gap-6">
            <Link 
              href="/#catalogo" 
              className="text-xs uppercase tracking-widest font-semibold text-gray-200 hover:text-[#D4AF37] transition-all duration-300 flex items-center gap-1.5 px-3.5 py-1.5 rounded-full hover:bg-[#D4AF37]/15"
            >
          ...
          <div className="flex items-center gap-4">
              ...
              {showSearch ? (
                <div className="flex items-center bg-black/90 border border-[#D4AF37]/60 rounded-full px-3 py-1 animate-in fade-in zoom-in-95">
                  <input
                    ...
                    className="bg-transparent text-xs text-white placeholder-gray-400 focus:outline-none w-32 md:w-44"
                    autoFocus
                  />
            ...
            {user ? (
              <div className="hidden md:flex items-center gap-3">
                <Link 
                  href="/comunidad" 
                  className="text-[11px] font-bold tracking-widest uppercase text-[#D4AF37] border border-[#D4AF37]/40 px-3.5 py-1 rounded-full hover:bg-[#D4AF37] hover:text-black transition-all"
                >
                  MI CLUB
                </Link>
              ...
              <Link 
                href="/login" 
                className="hidden md:flex items-center gap-1.5 text-xs tracking-wider uppercase font-semibold text-gray-200 hover:text-[#D4AF37] transition-colors"
              >
            ...
            >
              ...
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[#D4AF37] text-black font-extrabold text-[10px] w-4.5 h-4.5 rounded-full flex items-center justify-center shadow-[0_0_12px_rgba(212,175,55,0.9)] animate-pulse">
                  {cartItemCount}
                </span>
              )}
            </button>
